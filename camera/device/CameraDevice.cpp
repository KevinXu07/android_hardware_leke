...
namespace {
Mutex gMemoryOwnerLock;
wp<CameraDevice> gMemoryOwners[4];
bool gMemorySlotsUsed[4] = {};
}

// Stock L05 HAL passes QCameraStreamMemory*, not the framework cookie.
template<int Slot>
camera_memory_t* CameraDevice::sGetMemoryBound(int fd, size_t size, uint_t count, void*) {
    sp<CameraDevice> owner;
    {
        Mutex::Autolock lock(gMemoryOwnerLock);
        owner = gMemoryOwners[Slot].promote();
    }
    if (owner == nullptr) return nullptr;
    return sGetMemory(fd, size, count, owner.get());
}
    ...
    if (mDevice->ops->set_callbacks) {
        {
            Mutex::Autolock lock(gMemoryOwnerLock);
            for (int i = 0; i < 4; ++i) {
                if (!gMemorySlotsUsed[i]) {
                    gMemorySlotsUsed[i] = true;
                    gMemoryOwners[i] = this;
                    mMemorySlot = i;
                    break;
                }
            }
        ...
        static const camera_request_memory callbacks[] = {
            sGetMemoryBound<0>, sGetMemoryBound<1>, sGetMemoryBound<2>, sGetMemoryBound<3>
        };
        ALOGI("L05 memory owner: camera %s slot %d", mCameraId.c_str(), mMemorySlot);
        mDevice->ops->set_callbacks(mDevice,
                sNotifyCb, sDataCb, sDataCbTimestamp, callbacks[mMemorySlot], this);
    }
...
void CameraDevice::closeLocked() {
    ...
    // Release only after HAL close has joined its callback workers.
    if (mMemorySlot >= 0) {
        Mutex::Autolock lock(gMemoryOwnerLock);
        gMemoryOwners[mMemorySlot].clear();
        gMemorySlotsUsed[mMemorySlot] = false;
        mMemorySlot = -1;
    }
}
