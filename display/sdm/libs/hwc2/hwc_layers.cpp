...
HWC2::Error HWCLayer::SetLayerBuffer(buffer_handle_t buffer, int32_t acquire_fence) {
  ...
  // The scanout framebuffer may contain two pages in one dma-buf. Its
  // per-page offset is therefore allowed to exceed the size of one page.
  // Do not turn the second page into page zero; that corrupts composition.
  const bool is_framebuffer =
      (handle->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) != 0;
  uint32_t offset = handle->offset;
  if (!is_framebuffer && !layer_buffer->flags.video && offset >= handle->size) {
    DLOGW("Invalid UI buffer offset 0x%x (size 0x%x), using zero", offset,
          handle->size);
    offset = 0;
  }
  layer_buffer->planes[0].offset = offset;
  // MDSS scanout consumes the gralloc allocation stride, not the visible
  // width. L05's 1200-wide framebuffer is aligned to 1216 pixels.
  layer_buffer->planes[0].stride = UINT32(aligned_width);
